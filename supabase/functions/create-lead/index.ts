...
async function extractInvokeErrorMessage(error: unknown): Promise<string> {
  const fallback = "Failed to send verification email";
  ...
  if (context?.json) {
    try {
      ...
      // Ignore JSON parse issues and continue to text/message fallback.
    }
  }
  ...
  if (context?.text) {
    try {
      ...
      // Ignore text parsing issues and continue to message fallback.
    }
  }
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (upsertError) {
      ...
      throw new Error(
        `Failed to save lead information: ${upsertError.message}`,
      );
    }
    ...
    if (emailError || emailData?.success === false) {
      const emailFailureMessage = emailError
        ? await extractInvokeErrorMessage(emailError)
        : (emailData?.error as string) ||
          "Failed to send Your verification  email";
      console.error("Error sending verification email:", {
        invokeError: emailError,
        responseData: emailData,
        message: emailFailureMessage,
      });
      throw new Error(
        `Failed to send verification email: ${emailFailureMessage}`,
      );
    }
